...
local FONT_DIR     = "pluhtaki"
local FONTS_SUB    = FONT_DIR .. "/fonts"
if not isfolder(FONT_DIR)    then makefolder(FONT_DIR)    end
if not isfolder(FONTS_SUB)    then makefolder(FONTS_SUB)   end

local rawPath      = FONTS_SUB .. "/fs-tahoma-8px.ttf"
if not isfile(rawPath) then
    writefile(rawPath, game:HttpGet(
...
local fontDef = {
  ...
  faces = {{
    ...
    weight  = 400,
    style   = "normal",
    assetId = getcustomasset(rawPath),
  }},
}

local encPath = FONTS_SUB .. "/fs-tahoma-encoded.ttf"
if not isfile(encPath) then
  writefile(encPath, HttpService:JSONEncode(fontDef))
end
    ...
    silent_aim_key = Enum.KeyCode.Q,
    silent_aim_fov = 150,
    silent_aim_show_circle = true,
    silent_aim_team_check = true,
    silent_aim_target_part = "Head",
    silent_aim_prediction = true,
    silent_aim_strength = 0.1325,
    ...
    local config_flags = library.config_flags
     ...
     local themes = {
        preset = {
            ...
            ["inline"] = rgb(60, 55, 75), 
            ["accent"] = hex("#942a54"), 
            ["high_contrast"] = rgb(18, 3, 10),
            ["low_contrast"] = rgb(26, 5, 14),
...
library.font = fontFace
    ...
    local raw = listfiles(self.directory .. "/configs")
    local list = {}
    ...
    for key, v in pairs(config) do
        ...
        if not setter then
            warn("no setter for flag", key)
        else
            if type(v) == "table" then
                if     v.Color and v.Transparency then
                    setter(Color3.fromHex(v.Color), v.Transparency)
                elseif v.active ~= nil then
                    setter(v.active)
                else
        ...
        function library:panel(options) 
            local cfg = {
                ...
                size = options.size or dim2(0, 750, 0, 780),
...
MakeDraggable(library.keybind_list_frame)
                local inline = library:create("Frame", {
                    Parent = outline,
                    Name = "",
...
function MakeDraggable(GuiObject)
    ...
    MakeDraggable(watermark_outline)
     ...
     function library:playerlist(options) 
            local cfg = {
                players = {},
                 parent = options.parent or  cfg.parent, 
                callback = options.callback or function() end, 
                selected_player, 
